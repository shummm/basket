<script>
  import BasketModalEdit from './BasketModalEdit'
  ...
  export default {
    ...
    methods: {
      ...
      handlerClickDeleting() {
        this.$store.commit('basket/deleteItemsFake', this.checked)
        this.checked = []
      },
      handleClickInput(item) {
        this.currentModal = item.id
  ...
  <div class="page">
    ...
    <div class="basket">
      ...
      <div class="basket__inner basket__inner--headline">
        <div class="basket__item">
          <div class="basket__item--text">
            Выделелно
            <span class="basket__item--text-b">{{checked.length}}</span>
          </div>
          <div class="basket__item--text">
              <span class="basket__item--img" @click="handlerClickDeleting">
      ...
      <div
        ...
        :key="item.id"
        class="basket__inner basket__inner--row">
...
<path fill-rule="evenodd"  fill="rgb(0, 98, 150)"
      d="M5.827,8.274 C5.827,8.274 8.786,3.156 14.043,0.950 C10.855,5.876 9.145,8.121 5.873,13.256 C3.557,10.395 2.081,7.233 0.174,6.552 L5.827,8.274 Z"/>
        ...
        <div class="basket__item basket__item--col3 border--dotted-y">
          <div class="basket__item--text p--l28">{{item.description}}</div>
        </div>
        <div class="basket__item basket__item--col4">
          ...
          <input
            :value="item.quantity"
            @click.prevent.stop="handleClickInput(item)"
            type="text"
            class="basket__item--input"/>
          <basket-modal-edit
            :class="{'modal-active': item.id === currentModal}"
            @clickOut.stop="clickOut"
            @close="close"/>
